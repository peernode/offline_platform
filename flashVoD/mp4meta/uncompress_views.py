...
def update_uncompress(request):
    ...
    if request.method=='POST':
        ...
        try:
            ...
            obj=uncompress(Date=create_date,
                            Version=ver,
                            Total=decodes['total'],
                            Fail=decodes['fail'])
            obj.save()               
...
def get_uncompress_fail_ratio_contents(begin_date, end_date, ver):
    ...
    for i in ver_list:
        ...
        objs = uncompress.objects.filter(Version=ver_temp, Date__gte=begin_date, Date__lte=end_date).order_by("Date")
        for obj in objs:
            item={}
            item['date']=obj.Date
...
def make_uncompress_fail_ratio_contents(input):
    contents=[]
    item_idx=1
    for i in input.keys(): # beta, master          
        ...
        for idx in item_keys:
            ...
            serie_item='''{
                ...
                color: '%s',
                data: [%s]
            }'''%(idx, yAxis, show_format, color, ",".join(ylist))
